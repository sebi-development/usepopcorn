...
export default function ActionSection({ media, userRating, averageScore, isWatchlisted, isFavorited, onRate, onDelete }) {
    ...
    <div className="[grid-area:rating] flex flex-col items-center justify-center py-6 gap-8 w-full">
      ...
      <div className="w-full flex flex-col items-center gap-2">
        <button
          ...
          className="px-6 py-2.5 rounded-xl border border-white/10 text-sm font-medium text-text-muted hover:text-white hover:bg-white/5 hover:border-white/20 transition-all duration-200 cursor-pointer"
          style={{
            opacity: isRatingMode ? 0 : 1,
            pointerEvents: isRatingMode ? 'none' : 'auto',
            transform: isRatingMode ? 'scale(0.95)' : 'scale(1)',
        ...
        {isRatingMode && (
          <div className="flex justify-center pt-1 animate-in fade-in zoom-in-95 duration-200">
            <div
              ref={ratingPanelRef}
